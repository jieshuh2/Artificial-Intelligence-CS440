...
class TabQPolicy(QPolicy):
    ...
    def qvals(self, states):
        ...
        """
        qval = np.zeros((2, len(states)))
        for i in range(len(states)):
            dstate = self.discretize(states[i])
            for a in range(self.actionsize):
                qval[a, i] = self.model[dstate][a]
        return [qval]
    ...
    def td_step(self, state, action, reward, next_state, done):
        ...
        if (not done):
            ...
            qval = self.model[dstate][action]
            dnstate = self.discretize(next_state)
            qnext = max(self.model[dnstate][0], self.model[dnstate][1])
            target = reward + qnext*self.gamma
            self.model[dstate][action] = qval + self.lr*(target - qval)
            loss = (qval - target)**2
        else:
            loss = 0
        return loss
